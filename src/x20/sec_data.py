"""Look up SEC company facts and convert them into X20 inputs."""
...
def empty_fundamentals(symbol: str, reason: str = "loading", *, units: str = "USD") -> dict[str, object]:
    return {
        ...
        "interpretation": f"Fundamental data is not available yet: {reason}.",
        "normalized": {},
    }
...
def _interpret(revenue_growth: float | None, rd_intensity: float, operating_margin: float, capex_intensity: float) -> str:
    if revenue_growth is None:
        growth = "Revenue growth could not be calculated"
    elif revenue_growth >= 0:
        growth = f"Revenue grew {revenue_growth:.1%} from the comparable period"
    else:
        growth = f"Revenue fell {abs(revenue_growth):.1%} from the comparable period"
    return (
        f"{growth}. Operating margin was {operating_margin:.1%}. "
        f"R&D was {rd_intensity:.1%} of revenue, and capex was {capex_intensity:.1%}. "
        "These figures describe spending and operating results; they do not show by themselves whether the investment will pay off."
    )
