"""Ticker handling and the Alpaca IEX market-data client."""
...
def symbol_context(symbol: str) -> InstrumentContext:
    """Return the exchange, currency, and regulator implied by a ticker."""
...
class AlpacaIEXFeed:
    """Read snapshots and streaming events from Alpaca's IEX feed."""
        ...
        *,
        ...
        if not context.alpaca_live_supported:
            raise MarketDataConfigurationError(
                f"Alpaca IEX does not cover {self.symbol} on {context.exchange}. "
                "Use demo mode for Canadian tickers until a licensed TSX feed is connected."
            )
        ...
        if not self.key_id or not self.secret_key:
            raise MarketDataConfigurationError(
                "Set APCA_API_KEY_ID and APCA_API_SECRET_KEY before starting Alpaca IEX live mode."
            )
    ...
    def bootstrap(self) -> dict[str, object]:
        """Check IEX access and return the latest snapshot."""
    ...
    def run(self, stop: threading.Event) -> None:
        ...
        self.on_status("connecting", "opening the Alpaca IEX WebSocket")
        with connect(self.websocket_url, open_timeout=15, close_timeout=5) as socket:
            with self._socket_lock:
                self._socket = socket
            self.on_status("connected", "Alpaca WebSocket connected")
            ...
            while not stop.is_set():
                ...
                for event in events:
                    ...
                    if kind == "success" and event.get("msg") == "authenticated":
                        authenticated = True
                        self.on_status("authenticated", "Alpaca accepted the credentials")
                    ...
                    if kind == "subscription":
                        self.on_status("subscribed", f"subscribed to IEX data for {self.symbol}")
