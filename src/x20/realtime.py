"""Keep the current market state and rebuild model snapshots."""
...
class RealtimeEngine:
    ...
    def _alpaca_loop(self) -> None:
        while not self._stop.is_set():
            ...
            try:
                ...
                self._status("snapshot_ready", f"loaded the latest IEX snapshot for {symbol}")
                feed.run(self._stop)
                ...
                if generation == self._generation:
                    with self._lock:
                        self.fundamentals = empty_fundamentals(
                            symbol,
                            "SEDAR+ fundamentals have not been connected for Canadian companies",
    ...
    def snapshot(self) -> dict[str, object]:
        ...
        return {
            ...
            "limits": [
                "Canadian tickers use simulated prices until a licensed TSX feed is connected." if instrument.country == "CA" else "Alpaca IEX is a real exchange feed, but it does not include the full US SIP.",
                "The current coefficients are starting assumptions and have not passed walk-forward calibration.",
                "Inputs without a connected source stay at zero; the program does not fill them with estimates.",
                "These results are for research. They are not an order or a promise of returns.",
            ],
        }
