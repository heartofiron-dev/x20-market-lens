"""The 20-factor quadratic model used by X20."""
...
class QuadraticSignalModel:
    """Evaluate the quadratic score and its derivatives.
    ...
    def __init__(self) -> None:
        ...
        # Negative diagonal terms reduce a factor's effect near the ends of its range.
        for name, value in {
            "short_momentum": -0.12,
            "news_sentiment": -0.10,
            "revenue_growth": -0.10,
    ...
    def stress_delta(self, factors: Mapping[str, float], shock: Mapping[str, float]) -> dict[str, float]:
        """Calculate first- and second-order changes for a supplied shock."""
        x = self.vector(factors)
