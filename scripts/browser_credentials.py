"""Collect Alpaca credentials on localhost and start X20 live mode."""
...
<main>
  ...
  <p>请从 Alpaca Dashboard 复制当前的 Key 和 Secret。不要把它们发到聊天里。</p>
  <div class="safe">这两个值只会通过 127.0.0.1 交给本机 X20 进程，不会写进文件或 Git。</div>
  {notice}
  ...
  <small>连接成功后，页面会自动打开 http://127.0.0.1:8765/。</small>
</main>
</body>
</html>""".encode("utf-8")
...
def main() -> None:
    ...
    print(f"Open this local page to enter Alpaca credentials: http://{HOST}:{SETUP_PORT}/", flush=True)
